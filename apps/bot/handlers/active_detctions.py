...
async def active_detections_list_callback(callback_query: types.CallbackQuery, callback_data: PaginationCallbackData,
                                          state: FSMContext):
    if callback_data.name == 'next' or callback_data.name == 'previous':
        reply_markup = await inline_active_detections_pagination_keyboard(page=int(callback_data.page),
...
async def active_detections_detail_callback(callback_query: types.CallbackQuery, callback_data: PaginationCallbackData,
                                            state: FSMContext):
    data = await state.get_data()
    detection_id = data['detection_id']
    detection = await Detection.objects.filter(id=detection_id).select_related('user', 'car_model',
                                                                               'car_brand').afirst()
    text = f"""Deteksiya haqida ma'lumot:
Brand: <b>{detection.car_brand.name}</b>
Model: <b>{detection.car_model.name}</b>
Holati: {"Faol" if detection.is_active else "Nofaol"}
"""
...
async def active_detection_status_callback(callback_query: types.CallbackQuery, state: FSMContext):
    if callback_query.data == 'detection_status':
        data = await state.get_data()
        detection = await Detection.objects.filter(id=data['detection_id']).select_related('user', 'car_model',
                                                                                           'car_brand').afirst()
