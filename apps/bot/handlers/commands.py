...
async def start_commond(message: types.Message, state: FSMContext):
    ...
    if not user:
        await message.answer(text=" Avval regstratsiyadan o'ting", reply_markup=types.ReplyKeyboardRemove())
        await message.answer(text="Til:", reply_markup=inline_languages())
        await state.set_state(RegistrationStateGroup.language)
    else:
        await message.answer(f"Bot ishga tushdi <b>{user.first_name}</b>", reply_markup=reply_main_menu())