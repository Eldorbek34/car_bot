...
async def inline_pagination_keyboard(pagination_data, page: int = 0):
    ...
    for data in pagination_data[start_offset:end_offset]:
        try:
            inline_keyboard.row(InlineKeyboardButton(text=data.name,
                                                     callback_data=PaginationCallbackData(name=data.slug,
                                                                                          page=page).pack()))
        except Exception as err:
            name = f"{data.car_brand.name}({data.car_model.name})"
            if data.is_active:
                inline_keyboard.row(InlineKeyboardButton(text=f"{name}🟢",
                                                         callback_data=PaginationCallbackData(name=str(data.id),
                                                                                              page=page).pack()))
            else:
                inline_keyboard.row(InlineKeyboardButton(text=f"{name}🔴",
                                                         callback_data=PaginationCallbackData(name=str(data.id),
...
async def inline_active_detections_pagination_keyboard(telegram_id, page: int = 0):
    detections = await sync_to_async(list)(
        Detection.objects.filter(user_id=telegram_id).select_related("user", "car_brand", "car_model"))
